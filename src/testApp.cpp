...
void testApp::setup(){
    ...
    squirrelModel.loadModel("shoes/untitled.3ds", 500);

    
    squirrelModel.setScale(1.5, 1.5, 1.5);
...
void testApp::draw(){
	...
	ofDrawBitmapString(ofToString(mouseY+7) + " cm ", 10, 40);
    if(mouseY < 0){
        mouseY = 0;
    }
	// Threshold image
	...
	if(myIndex >= 0) {	
        ...
        glTranslatef(1000,150,0);
        //glRotatef(-mouseY,1,0,0);
        glRotatef(mouseX,0,1,0);
//scale
        glScalef(1, mouseY*0.1, 1);
        squirrelModel.draw();
        ofPopMatrix();
	}
